...
<xsl:text>\label{</xsl:text><xsl:value-of select="normalize-space($id)"/><xsl:text>}</xsl:text>
<xsl:if test="$latex.use.hyperref=1 and local-name($object)!='article' and local-name($object) != 'book'">
	<xsl:text>\hypertarget{</xsl:text><xsl:value-of select="normalize-space($id)"/>
	<xsl:text>}{}&#10;</xsl:text>
</xsl:if>
<xsl:text>&#10;</xsl:text>
</xsl:template>
